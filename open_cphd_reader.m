...
for i = 1:xml_meta.Data.NumCPHDChannels
    ...
    for j = 1:numel(vectorParametersCell)
        fld = vectorParametersCell{j};

        % normal (non-added) PVP entries
        if ~strcmp(fld, 'AddedPVP')
            vpdef = xml_meta.PVP.(fld);  % has Offset and Size
            vbp_all(i).(fld) = ...
                vb_array(vpdef.Offset + (1:vpdef.Size), :).';
        else
            % AddedPVP can be a struct OR a cell array depending on producer
            added = xml_meta.PVP.AddedPVP;

            % normalize to cell
            if ~iscell(added)
                % if it's a struct array, turn it into cell array
                added = num2cell(added);
            end

            for jj = 1:numel(added)
                name = added{jj}.Name;
                off  = added{jj}.Offset;
                sz   = added{jj}.Size;
